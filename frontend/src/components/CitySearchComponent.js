...
export default function CitySearchComponent() {
  ...
  useEffect(() => {
    loadPollenDataAccordingToResponsePosition(location);
  }, [location]);

  function loadPollenDataAccordingToResponsePosition(location) {
    console.log(pollenCount[0]?.content[location].Pollen);
  }
